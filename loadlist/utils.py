...
def semestre():
    ...
    if int(now.strftime("%m")) < 3:
        ...
        newDir = "II_TRIM_" + year + "/"
    elif int(now.strftime("%m")) >6 and int(now.strftime("%m")) < 9:
        newDir = "III_TRIM_" + year + "/"
    else:
        newDir = "IV_TRIM_" + year + "/"
...
def crearCoordinadorFolder():
    ...
    try:
        ...
        # crear Testing directorio si no existe
def crearTestingFolder():
    newDir = semestre()
    try:
        os.makedirs(testing_destiny_path + newDir)
        endDir = testing_destiny_path + newDir
        return endDir
    except:
        endDir = testing_destiny_path + newDir
        return endDir
...
def clean_data_instructor(dataframe):
    ...
    dataframe['NUMERO_DE_DOCUMENTO_INSTRUCTOR'] = dataframe['NUMERO_DE_DOCUMENTO_INSTRUCTOR'].astype(str)
    dataframe['NUMERO_DE_DOCUMENTO_INSTRUCTOR'].ffill(axis = 0)
    ...
    # Clean columns names
def clean_data_coordinador(dataframe):
